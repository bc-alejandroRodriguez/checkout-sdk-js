...
export function getWorldpayPaymentMethodMock(): PaymentMethod {
    const paymentMethodMock = getPaymentMethodMock();
...
export function getWorldpayPaymentDataRequest(): GooglePayPaymentDataRequestV2 {
    return {
        apiVersion: 2,
        apiVersionMinor: 0,
        merchantInfo: {
...
export function getWorldpayTokenizedPayload(): TokenizePayload {
    return {
        nonce: btoa(
            '{"signature":"foo","protocolVersion":"ECv1","signedMessage":"{"encryptedMessage":"foo","ephemeralPublicKey":"foo"}"}',
        ),
...
export function getWorldpayPaymentDataMock(): GooglePaymentData {
    const googlePaymentDataMock = getGooglePaymentDataMock();
